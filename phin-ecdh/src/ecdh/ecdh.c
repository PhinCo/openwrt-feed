...
EVP_PKEY *pemToKey(unsigned char *peerPublicKey, int peerPublicKeyLength) {
...
int generateKeyPair(KeyPair *kp) {
	...
	if (1 != EVP_PKEY_keygen(kctx, &(kp->privateKey))) {
		return 1;
	}
    ...
    BIO* bp = BIO_new(BIO_s_mem());
    //Create the K key.
    if (1!=  PEM_write_bio_PUBKEY(bp, kp->privateKey)){
    ...
    kp->publicKey = (unsigned char *) malloc(sizeof (unsigned char) * bptr->length);
	int len = (int) bptr->length;
    memcpy(kp->publicKey, bptr->data, bptr->length);
...
unsigned char *deriveSecret(EVP_PKEY *privateKey, unsigned char *publicKey, int publicLength, size_t *secret_length) {
	...
	if(NULL == (ctx = EVP_PKEY_CTX_new(privateKey, NULL))) {
		return NULL;
	}
	...
	if(1 != EVP_PKEY_derive_set_peer(ctx, pemToKey( publicKey, publicLength))) {
		return NULL;
	}
