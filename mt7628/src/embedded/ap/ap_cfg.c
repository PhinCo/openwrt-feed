...
INT Show_txpower_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
INT Set_ApCli_Macaddress_Proc(PRTMP_ADAPTER pAd, RTMP_STRING *arg);
...
static struct {
	...
	{"ApCliMacaddress",				Set_ApCli_Macaddress_Proc},
	...
	IN  RTMP_STRING *arg)
{
	...
	NdisZeroMemory(pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr, MAC_ADDR_LEN);
	...
	if(strlen(arg) == 17)  /* Mac address acceptable format 01:02:03:04:05:06 length 17 */
	{
		for (i=0, value = rstrtok(arg,":"); value; value = rstrtok(NULL,":"), i++)
		{
			...
			AtoH(value, &pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[i], 1);
		}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ApCli_Macaddress_Proc (%2X:%2X:%2X:%2X:%2X:%2X)\n",
		pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[0],
		pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[1],
		pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[2],
		pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[3],
		pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[4],
		pAd->ApCfg.ApCliTab[ifIndex].wdev.if_addr[5]));
