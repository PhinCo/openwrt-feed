...
typedef struct _vif
{
    ...
    param encryption;		/* ccmp(aes),tkip */
    param key;             /* wpa psk */
...
vif VIF =
{
    .mode				= {NULL, "mode", {0}, NULL,  NULL},
    .disabled           = {NULL, "disabled", {0}, NULL,  NULL},
    .macaddr			= {NULL, "macaddr", {0}, NULL,  NULL},
    .ssid               = {NULL, "ssid", {0}, NULL,  NULL},
    .bssid              = {NULL, "bssid", {0}, NULL,  NULL},
    .hidden             = {NULL, "hidden", {0}, NULL,  NULL},
    .encryption			= {NULL, "encryption", {0}, NULL,  NULL},
...
param CFG_ELEMENTS[] =
{
    ...
    */
    ...
    {"WirelessMode", "hwmode", {0}, hooker,  "9"},
    ...
    {"HT_BSSCoexistence", "noscan", {0}, hooker, "0"},
    ...
    {"ApCliEnable", NULL, {0}, hooker, "0"},
	{"ApCliMacaddress", NULL, {0}, hooker, "0"},
    {"ApCliSsid", NULL, {0}, hooker, NULL},
    {"ApCliBssid", NULL, {0}, hooker, NULL},
...
void parse_uci(char * arg)
{
    ...
    uci_foreach_element(&uci_wireless->sections, e) {
        ...
        if (!strcmp(s->type, "wifi-device")) {
            ...
            for (cur_dev = 0; cur_dev < DEVNUM_MAX; cur_dev++) {
                if(!strcmp(s->e.name, wifi_cfg[cur_dev].devname))
                    break;
            }
    ...
    uci_foreach_element(&uci_wireless->sections, e) {
        ...
        if (!strcmp(s->type, "wifi-iface")) {
			...
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].bssid, value);
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].hidden, value);
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].encryption, value);
...
void hooker(FILE * fp, param * p, const char * devname)
{
    ...
    for (N = 0; N < MBSSID_MAX; N++) {
        if(!strcmp(devname, wifi_cfg[N].devname))
            break;
    }
    ...
    if(!strmatch(p->dat_key, "SSID?")) {
    ...
    } else if (!strcmp(p->dat_key, "AuthMode")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			if (strcmp(wifi_cfg[N].vifs[i].mode.value, "ap"))
				continue;
    ...
    } else if (!strcmp(p->dat_key, "Channel")) {
        if(!strcmp(p->value, "auto"))
            FPRINT(fp, p, "0");
        else
            FPRINT(fp, p, "%s", p->value);
	} else if (!strcmp(p->dat_key, "TxPower")) {
		int txpower = dbm2mw(atoi(p->value));
		if (txpower > 100) {
			FPRINT(fp, p, "%d", 100);
		} else {
			FPRINT(fp, p, "%d", txpower);
		}
    } else if (!strcmp(p->dat_key, "AutoChannelSelect")) {
        if(!strcmp(p->value, "auto"))
            FPRINT(fp, p, "2");
        else
            FPRINT(fp, p, "0");
    } else if (!strcmp(p->dat_key, "HT_BW")) {
        if (!strcmp(p->value, "HT40") || !strcmp(p->value, "VHT40"))
            FPRINT(fp, p, "1");
        else
            FPRINT(fp, p, "0");
    } else if (!strcmp(p->dat_key, "VHT_BW")) {
        if (!strcmp(p->value, "VHT80"))
            FPRINT(fp, p, "1");
        else
            FPRINT(fp, p, "0");
	} else if (!strcmp(p->dat_key, "HT_BSSCoexistence")) {
		if (!strcmp(p->value, "1")) {
			FPRINT(fp, p, "0");
		} else {
			FPRINT(fp, p, "1");
		}
	} else if (!strcmp(p->dat_key, "WirelessMode")) {
		if (!strcmp(p->value, "11b")) {
			FPRINT(fp, p, "1");
    ...
    } else if(!strmatch(p->dat_key, "WPAPSK?")) {
		...
		if (wifi_cfg[N].vifs[i].key.value)
			FPRINT(fp, p, "%s", wifi_cfg[N].vifs[i].key.value);
    } else if (!strcmp(p->dat_key, "RADIUS_Server")) {
        for (i = 0; i < wifi_cfg[N].vifnum; i++) {
            if (i > 0) 
				FPRINT(fp, p, ";");
    ...
    } else if (!strcmp(p->dat_key, "RADIUS_Port")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
            if(i > 0) 
				FPRINT(fp, p, ";");
    ...
    } else if (!strmatch(p->dat_key, "RADIUS_Key?")) {
    ...
    } else if (!strcmp(p->dat_key, "PreAuth")) {
        for (i = 0; i < wifi_cfg[N].vifnum; i++) {
            if (i > 0) 
				FPRINT(fp, p, ";");
    ...
    } else if (!strcmp(p->dat_key, "RekeyInterval")) {
        for (i = 0; i < wifi_cfg[N].vifnum; i++) {
            if (i > 0) 
				FPRINT(fp, p, ";");
    ...
    } else if (!strcmp(p->dat_key, "PMKCachePeriod")) {
        for (i = 0; i < wifi_cfg[N].vifnum; i++) {
            if (i > 0) 
				FPRINT(fp, p, ";");
    ...
    } else if (!strcmp(p->dat_key, "RekeyMethod")) {
        FPRINT(fp, p, "%s", p->defvalue);
#if 0
    } else if (!strcmp(p->dat_key, "IEEE8021X")) {
        for (i = 0; i < wifi_cfg[N].vifnum; i++) {
            if (i > 0) 
				FPRINT(fp, p, ";");

            if (!strcasecmp(wifi_cfg[N].vifs[i].authmode.value, "8021x"))
                FPRINT(fp, p, "%s", "1");
        }
    } else if (!strmatch(p->dat_key, "DefaultKeyID")) {
    	for (i = 0; i < wifi_cfg[N].vifnum; i++) {
    ...
    else if(0 == strmatch(p->dat_key, "RADIUS_Key?"))  /* Radius */
    {
        ...
        /* TODO */
#endif
    } else if (!strmatch(p->dat_key, "ApCliMacaddress")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			if (strcmp(wifi_cfg[N].vifs[i].mode.value, "sta"))
				continue;

			FPRINT(fp, p, "%s", wifi_cfg[N].vifs[i].macaddr.value);
		}
	} else if (!strmatch(p->dat_key, "ApCliEnable")) {
		int enabled = 0;
        ...
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			...
			if (!strcmp(wifi_cfg[N].vifs[i].disabled.value, "1")) {
				FPRINT(fp, p, "%s", "0");
			} else {
				FPRINT(fp, p, "%s", "1");
			}
		...
		if (!enabled) 
			FPRINT(fp, p, "%s", "0");
	} else if (!strmatch(p->dat_key, "ApCliSsid")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			if (strcmp(wifi_cfg[N].vifs[i].mode.value, "sta"))
				continue;
	...
	} else if (!strmatch(p->dat_key, "ApCliBssid")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			if (strcmp(wifi_cfg[N].vifs[i].mode.value, "sta"))
				continue;
	...
	} else if (!strmatch(p->dat_key, "ApCliEncrypType")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			if (strcmp(wifi_cfg[N].vifs[i].mode.value, "sta"))
				continue;
	...
	} else if (!strmatch(p->dat_key, "ApCliWPAPSK")) {
        for(i = 0; i < wifi_cfg[N].vifnum; i++) {
			if (strcmp(wifi_cfg[N].vifs[i].mode.value, "sta"))
				continue;
